...
map(addTranslationLabel, Object.keys(settingsCategories).map(function (categoryName) {
...
var settingsCategoryItemMap = Object.keys(settingsCategories).map(getMenuItemForCategory);

export default function addDeepLinksForSettings(headerBarMenuItems) {
    var settingsItems$ = Observable.just(settingsCategoryItemMap);
